...
def train(epo, model, train_loader, optimizer):
    ...
    for it, (images, labels, names) in enumerate(train_loader):
        ...
        if it %200 ==0:
            print('Train: %s, epo %s/%s, iter %s/%s, lr %.8f, %.2f img/sec, loss %.4f, time %s' % (args.model_name, epo, args.epoch_max, it+1, len(train_loader), lr_this_epo, len(names)/(time.time()-start_t), float(loss), datetime.datetime.now().replace(microsecond=0)-start_datetime))
...
if __name__ == '__main__':
    ...
    train_dataset = RSUSS_dataset(data_dir=args.data_dir, split='train', transform=augmentation_methods)
    val_dataset  = RSUSS_dataset(data_dir=args.data_dir, split='val')
    test_dataset = RSUSS_dataset(data_dir=args.data_dir, split='test')
